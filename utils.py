# utils.py — v2 SMART ENGINE ONLY
# ---------------------------------------------------------------------------
# Complete utilities for WoodyTradesPro Forecast app
# Includes:
#   • Data fetch + cache via yfinance
#   • Technical indicators (EMA, RSI, MACD, ATR)
#   • Sentiment via Vader on Yahoo headlines
#   • Market regime detection (trend vs range)
#   • RandomForestClassifier probability engine
#   • Adaptive fused trading signal
#   • Realistic backtest
#   • Summary + per-asset pipelines
# ---------------------------------------------------------------------------
...
def fetch_data(symbol: str, interval_key: str="1h", use_cache=True) -> pd.DataFrame:
    if interval_key not in INTERVALS: raise KeyError(interval_key)
    ...
    if use_cache and cache_fp.exists():
        try:
            ...
            if len(df) >= min_rows: return _normalize(df)
        except: pass
    for _ in range(3):
        try:
    ...
    try:
        ...
        df = _normalize(raw)
        if not df.empty: df.to_csv(cache_fp)
        return df
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        atr=AverageTrueRange(df["High"],df["Low"],df["Close"],14); df["atr"]=atr.average_true_range()
    except Exception:
...
def fetch_sentiment(symbol: str) -> float:
    try:
        tk=yf.Ticker(symbol); news=tk.news; 
...
def detect_regime(df: pd.DataFrame) -> str:
    if "ema20" not in df: return "range"
...
def train_rf(df: pd.DataFrame) -> Optional[RandomForestClassifier]:
    ...
    df=df.copy(); df["fwd"]=df["Close"].pct_change().shift(-1); df["up"]=(df["fwd"]>0).astype(int)
    X=df[cols].shift(1); y=df["up"]
    mask=X.notna().all(axis=1)&y.notna()
    if mask.sum()<200: return None
    model=RandomForestClassifier(n_estimators=100,max_depth=4,random_state=42)
    model.fit(X[mask],y[mask]); return model
...
def compute_tp_sl(price: float, atr: float, side: str, risk: str) -> Tuple[float,float]:
    ...
    if side=="Buy": return price+m["tp_atr"]*atr, price-m["sl_atr"]*atr
    else: return price-m["tp_atr"]*atr, price+m["sl_atr"]*atr

def fused_signal(df: pd.DataFrame, symbol: str, risk: str="Medium") -> Optional[Dict[str,object]]:
    if df.empty: return None
    df=add_indicators(df); sent=fetch_sentiment(symbol); df["sentiment"]=sent; regime=detect_regime(df)
    model=train_rf(df)
    ...
    elif last["macd"]<last["macd_signal"]: score-=1
    side_rule="Buy" if score>0 else "Sell" if score<0 else "Hold"
    # ML prob
    prob_up=0.5
    if model is not None:
        feat=np.array([[last.get("ema20"),last.get("ema50"),last.get("RSI"),last.get("macd"),last.get("macd_signal"),last.get("atr"),sent]],dtype=float)
        if not np.isnan(feat).any(): prob_up=float(model.predict_proba(feat)[0,1])
    votes_buy=votes_sell=0
    if side_rule=="Buy": votes_buy+=1
    elif side_rule=="Sell": votes_sell+=1
    if sent>0.1: votes_buy+=1
    elif sent<-0.1: votes_sell+=1
    if prob_up>0.55: votes_buy+=1
    elif prob_up<0.45: votes_sell+=1
    ...
    atr=float(last["atr"]); price=float(last["Close"]); tp,sl=compute_tp_sl(price,atr,final_side,risk)
    return {"side":final_side,"prob":conf,"price":price,"tp":tp,"sl":sl,"atr":atr,"regime":regime,"sentiment":sent,"ml_prob_up":prob_up}
...
def backtest(df: pd.DataFrame, symbol: str, risk: str="Medium") -> Dict[str,object]:
    ...
    if len(df)<120: return res
    df=add_indicators(df); df["sentiment"]=fetch_sentiment(symbol)
    pos=None; wins=ret_sum=0; trades=[]
    for i in range(60,len(df)):
        sig=fused_signal(df.iloc[:i+1],symbol,risk)
        if not sig: continue
        side=sig["side"]; px=float(df["Close"].iloc[i]); ts=df.index[i]
        if pos is None and side in("Buy","Sell"): pos=(side,px,ts)
        elif pos is not None:
            ps,ep,ets=pos
            if side in("Buy","Sell") and side!=ps:
                r=(px-ep)/ep*(1 if ps=="Buy" else -1); ret_sum+=r
                if r>0:wins+=1
                trades.append({"entry_time":ets,"exit_time":ts,"side":ps,"entry":ep,"exit":px,"return_pct":r*100})
                pos=(side,px,ts)
    if pos is not None:
        ps,ep,ets=pos; lp=float(df["Close"].iloc[-1]); lts=df.index[-1]
        r=(lp-ep)/ep*(1 if ps=="Buy" else -1); ret_sum+=r
        if r>0:wins+=1
        trades.append({"entry_time":ets,"exit_time":lts,"side":ps,"entry":ep,"exit":lp,"return_pct":r*100})
    n=len(trades)
    res.update({"n_trades":n,"win_rate":100*wins/n if n else 0,"total_return_pct":100*ret_sum,"trades":trades})
    return res
...
def analyze_asset(symbol: str, interval_key: str, risk: str="Medium", use_cache=True) -> Optional[Dict[str,object]]:
    ...
    return {"symbol":symbol,"interval_key":interval_key,"risk":risk,"last_price":float(df["Close"].iloc[-1]),
            "signal":pred["side"],"probability":round(pred["prob"]*100,2),"tp":pred["tp"],"sl":pred["sl"],"atr":pred["atr"],
            "regime":pred["regime"],"sentiment":pred["sentiment"],"ml_prob_up":round(pred["ml_prob_up"]*100,2),
            "win_rate":bt["win_rate"],"total_return_pct":bt["total_return_pct"],"n_trades":bt["n_trades"],"df":df,"trades":bt["trades"]}

def summarize_assets(interval_key: str="1h", risk: str="Medium", use_cache=True) -> pd.DataFrame:
    rows=[]; _log("Fetching and analyzing market data (smart v2)...")
    for asset,symbol in ASSET_SYMBOLS.items():
        _log(f"{asset} ({symbol})...")
        try:
            r=analyze_asset(symbol,interval_key,risk,use_cache)
            if r:
                rows.append({"Asset":asset,"Symbol":symbol,"Interval":interval_key,"Price":r["last_price"],
                             "Signal":r["signal"],"Probability_%":r["probability"],"TP":r["tp"],"SL":r["sl"],
                             "WinRate_%":r["win_rate"],"BacktestReturn_%":r["total_return_pct"],"Trades":r["n_trades"],
                             "Regime":r["regime"],"Sentiment":r["sentiment"],"ML_ProbUp_%":r["ml_prob_up"]})
...
def load_asset_with_indicators(asset: str, interval_key: str, use_cache=True) -> Tuple[str,pd.DataFrame]:
    if asset not in ASSET_SYMBOLS: raise KeyError(asset)
    sym=ASSET_SYMBOLS[asset]; df=fetch_data(sym,interval_key,use_cache)
...
def asset_prediction_and_backtest(asset: str, interval_key: str, risk: str, use_cache=True) -> Tuple[Optional[Dict[str,object]],pd.DataFrame]:
    ...
    if not sym: return None,pd.DataFrame()
    df=fetch_data(sym,interval_key,use_cache)
    if df.empty: return None,df
    df=add_indicators(df)
    pred=fused_signal(df,sym,risk); bt=backtest(df,sym,risk)
    if not pred: return None,df
    return {"asset":asset,"symbol":sym,"interval":interval_key,"price":float(df["Close"].iloc[-1]),"side":pred["side"],
            "probability":round(pred["prob"]*100,2),"tp":pred["tp"],"sl":pred["sl"],"atr":pred["atr"],
            "regime":pred["regime"],"sentiment":pred["sentiment"],"ml_prob_up":round(pred["ml_prob_up"]*100,2),
            "win_rate":bt["win_rate"],"backtest_return_pct":bt["total_return_pct"],"n_trades":bt["n_trades"],"trades":bt["trades"]},df
