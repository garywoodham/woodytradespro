...
def _log(msg: str) -> None:
    """Print-safe log for Streamlit console visibility."""
    try:
...
def _normalize_ohlcv(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure clean OHLCV DataFrame (fixes MultiIndex, ndarray columns, NaNs)."""
...
def fetch_data(
    ...
    """Fetch OHLCV data with caching and retries."""
    ...
    if use_cache and cache_fp.exists():
        try:
            ...
            if len(cached) >= min_rows:
                ...
                _log(f"ℹ️ Cache incomplete ({len(cached)} rows). Refetching...")
        except Exception as e:
    ...
    if not df.empty and len(df) >= min_rows:
        try:
            df.to_csv(cache_fp)
            _log(f"💾 Cached mirror data for {symbol}")
        except Exception as e:
            _log(f"⚠️ Cache write failed: {e}")
        return df
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Add EMA, RSI, MACD, ATR indicators to DataFrame."""
...
def compute_signal_row(row_prev: pd.Series, row: pd.Series) -> Tuple[str, float]:
    """Compute Buy/Sell/Hold signal based on EMA, RSI, MACD."""
    score, votes = 0.0, 0

    # EMA cross
    if pd.notna(row["ema20"]) and pd.notna(row["ema50"]):
        votes += 1
        score += 1 if row["ema20"] > row["ema50"] else -1 if row["ema20"] < row["ema50"] else 0
...
def latest_prediction(df: pd.DataFrame, risk: str = "Medium") -> Optional[Dict[str, object]]:
    ...
    tp, sl = compute_tp_sl(float(row["Close"]), atr_val, side, risk) if side != "Hold" else (None, None)
...
def backtest_signals(df: pd.DataFrame, risk: str = "Medium", hold_allowed: bool = True) -> Dict[str, object]:
    ...
    for idx in range(len(signals)):
        ...
        if exit_reason:
            ...
            trades.append({"entry_time": entry_ts, "exit_time": ts, "side": pos_side,
...
def analyze_asset(symbol: str, interval_key: str, risk: str = "Medium", use_cache: bool = True) -> Optional[Dict[str, object]]:
    ...
    if pred is None:
        return None
    bt = backtest_signals(df, risk=risk)
    return {"symbol": symbol, "interval_key": interval_key, "risk": risk,
            "last_price": float(df["Close"].iloc[-1]), "signal": pred["side"],
            "probability": round(pred["prob"] * 100.0, 2), "tp": pred["tp"], "sl": pred["sl"],
            "atr": pred["atr"], "win_rate": bt.get("win_rate"),
            "total_return_pct": bt.get("total_return_pct"), "n_trades": bt.get("n_trades"),
            "df": df, "trades": bt["trades"]}
...
def summarize_assets(interval_key: str = "1h", risk: str = "Medium", use_cache: bool = True) -> pd.DataFrame:
    ...
    if not rows:
        _log("No assets analyzed — data unavailable.")
        return pd.DataFrame()
...
def asset_prediction_and_backtest(asset: str, interval_key: str, risk: str, use_cache: bool = True) -> Tuple[Optional[Dict[str, object]], pd.DataFrame]:
    """Used by Streamlit tabs to fetch full prediction + backtest info."""
    ...
    pred_out = {"asset": asset, "symbol": symbol, "interval": interval_key,
                ...
                "probability": round(pred["prob"] * 100.0, 2), "tp": pred["tp"], "sl": pred["sl"],
                "atr": pred["atr"], "win_rate": bt.get("win_rate"),
                "backtest_return_pct": bt.get("total_return_pct"),
