...
# ───────────────────────────────────────────────
def _normalize(df):
    """Ensure numeric columns, consistent capitalization."""
...
def _mirror_fetch(symbol):
    """Mirror data fetch via Yahoo JSON endpoint."""
    try:
...
# ───────────────────────────────────────────────
# FETCH DATA (CACHE + RETRY + MIRROR)
# ───────────────────────────────────────────────
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_data(symbol, interval="1h", period="1mo", retries=4, force_refresh=False):
    """
    Fetch Yahoo Finance data robustly.
    Handles caching, retries, and mirror fallback.
    """
    ...
    if not force_refresh and os.path.exists(fpath):
        ...
        if age < timedelta(hours=48):  # 48h cache validity
            try:
                df = pd.read_csv(fpath, index_col=0, parse_dates=True)
                if not df.empty:
                    st.write(f"📁 Loaded cached {symbol} ({len(df)} rows, {age.seconds//3600}h old)")
                    return _normalize(df)
            except Exception as e:
    ...
    for attempt in range(1, retries + 1):
        try:
            st.write(f"⏳ Attempt {attempt}: Fetching {symbol} ({interval})...")
            data = yf.download(
                symbol,
                period=period,
                interval=interval,
            ...
            if isinstance(df, pd.DataFrame) and "Close" in df.columns and len(df) >= 50:
                ...
                st.warning(f"⚠️ {symbol}: insufficient or missing data ({len(df)} rows). Retrying...")

        except Exception as e:
            if "Too Many Requests" in str(e):
                wait = 10 + random.uniform(1, 5)
                st.warning(f"🕒 Rate limit hit for {symbol}. Waiting {wait:.1f}s...")
                time.sleep(wait)
            else:
    ...
    if not df.empty:
        try:
            df.to_csv(fpath)
            st.write(f"💾 Cached {symbol} → {fpath}")
        except Exception as e:
...
def add_indicators(df):
    """Add technical indicators for model use."""
    if df.empty:
        return df
    df = df.copy()
...
def apply_trading_theory(pred, df):
    """Overlay EMA/RSI-based bias for realism."""
    if df.empty:
        return pred, 1.0
    try:
...
def train_and_predict(df, horizon="1h", risk="Medium"):
    """Train RandomForest and produce prediction."""
    ...
    return {"prediction": pred, "probability": conf, "accuracy": round(acc, 3),
...
def backtest_signals(df, model_output):
    """Simulate simple EMA crossover backtest."""
...
def summarize_assets(force_refresh=False):
    ...
    if not results:
        st.error("🚫 No assets could be analyzed. Please check connection or source.")
        return pd.DataFrame()
